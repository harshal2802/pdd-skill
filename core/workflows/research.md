...
These notes should be reusable later in `plan`, `prompts`, and `review`.
...
- **Extend**: move to `prompts` with adaptation guidance
- **Compose**: move to `plan` or `prompts` depending on complexity
- **Build**: move to `plan` for multi-step work, or `prompts` if the task is still small and clear
...
- **No existing solutions**: confirm the scan was thorough, then proceed to `plan`
- **Problem is too vague**: narrow it to one use case or user story before researching broadly

## Default Next Step

If the recommendation is to build, move to `plan`. If the research settles a small change directly, move to `prompts`.
