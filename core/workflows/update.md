...
- recommend reviewing the new output with `review`
...
Start from scratch only when the prompt is fundamentally the wrong shape for the job. In that case, route back to `prompts` and preserve the old prompt as an experiment or historical reference.
