...
This is the Claude adapter for the shared `Init` workflow in `core/workflows/init.md`. Keep shared workflow behavior aligned there; this file exists to preserve Claude-specific command wording and `$ARGUMENTS` handling.
...
Init should preserve reality. It adds the PDD layer to an existing codebase, detects what already exists, and hands off into `/project:pdd-context` with a truthful starting point.
...
If the directory looks empty or brand new, route to `/project:pdd-scaffold` instead.
...
Move to `/project:pdd-context` and write what is true today, not what the team hopes to build later.
